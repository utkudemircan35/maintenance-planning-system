...
async function startWO(id) {
  // Database does not have a status column for work_orders
...
async function submitLog(e) {
  ...
  const newLog = {
    id: Date.now(), // generating integer id locally just in case
...
async function submitProduction(e) {
  ...
  const util = m ? Math.min(100, (vol / (100 * (hours/8))) * 100) : 75; // Using 100 as nominal since it's not in schema
  const newProd = {
    id: Date.now(),
    machine_id: mid,
...
function renderMaintenancePlans(el) {
  ...
  ${DEMO_MAINTENANCE_SCHEDULES.map(p=>{const m=getMachineById(p.machine_id); const risk = m ? RISK_SCORES[m.id] : 0; return `<tr>
...
function renderWOTable(orders) {
  ...
  ${orders.map(w=>{const m=getMachineById(w.machine_id);return `<tr>
...
async function submitWO(e) {
  ...
  const newWo = {
    id: 'WO-2024-' + String(DEMO_WORK_ORDERS.length+1).padStart(5,'0'),
    machine_id: document.getElementById('woMachine').value,
    machine_name: getMachineById(document.getElementById('woMachine').value)?.machine_name,
    technician: document.getElementById('woTech').value,
...
function renderAlertMgmt(el) {
  ...
  ${DEMO_ALERTS.map(a=>{const m=getMachineById(a.machine_id); const sev = a.alert_type === 'critical' ? 'Critical' : a.alert_type === 'warning' ? 'High' : 'Medium'; return `<tr>
    <td><strong>${a.id}</strong></td><td>${m?m.machine_name:a.machine_id}</td><td style="font-size:12px;">${a.alert_type}</td>
    <td><span class="badge bg-${sev==='Critical'?'danger':sev==='High'?'warning':sev==='Medium'?'info':'secondary'} badge-pill">${sev}</span></td>
...
function renderAuditLog(el) {
  ...
  ${DEMO_AUDIT_LOG.slice().reverse().map(l=>{const u=DEMO_USERS.find(x=>x.id===l.user_name || x.full_name===l.user_name);return `<tr>
